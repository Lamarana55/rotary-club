            ...
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <a class="btn bg-gradient-primary text-white" data-bs-toggle="modal"
                            data-bs-target="#showModelAddTypeDocument">
                            Ajouter un type
                        </a>
                        @endif
                    </div>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @forelse ($typeDocument as $key=>$user )
                        <tr>
                            <td>{{$key+1}} </td>
                            <td>{{ $user->nom }}</td>
                            <td>
                            ...
                            <td class="text-right">
                                ...
                                <a class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                    data-bs-target="#showModelEditTypeDocument"
                                    wire:click="getTypeDocument({{$user->id}})">Modifier</a>&nbsp;
                                @endif
                                @if (hasPermission('supprimer_type_document'))
                                <a class="btn btn-sm bg-gradient-danger"
                                    wire:click.prevent="confirmDeleteTypeDocument({{$user->id}})">Supprimer</a>&nbsp;
                                @endif
                            </td>
                        </tr>
                        @empty
...
<script>
    ...
    window.addEventListener("closeModal", event=>{
        $("#showModelEditTypeDocument").modal("hide")
    })
